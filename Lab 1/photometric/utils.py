...
def load_syn_images(image_dir = './SphereGray5/', channel = 0):
    ...
    normV = np.tile(
        np.sqrt(np.sum(V ** 2, axis=1, keepdims=True)), (1, V.shape[1])
    )
    scriptV = V / normV
...
def load_face_images(image_dir = './yaleB02/'):
    ...
    for j in range(num_images):
        ang[0, j], ang[1, j] = \
            np.double(filenames[j][12:16]), \
            np.double(filenames[j][17:20])
        image_stack[..., j] = cv2.imread(os.path.join(
            image_dir, filenames[j]), -1
        ) - ambient_image
...
def show_results(albedo, normals, height_map, SE):
    ...
    '''
    =============
    You could further inspect the shape of the objects and normal
    directions by using plt.quiver() function.
    =============
    '''
